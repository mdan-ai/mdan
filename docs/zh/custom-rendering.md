---
title: 自定义渲染
description: 在保留 MDSN 浏览器运行时的前提下，用 Vue 或 React 自己接管 UI。
---

# 自定义渲染

当你想保留 MDSN 的浏览器运行时，但希望由自己的框架完全接管 UI 时，就走这条路径。

## 共同原则

- `@mdsnai/sdk/web` 负责请求生命周期、状态更新和协议行为
...
参考：[examples/vue-starter/app/client.ts](/Users/hencoo/projects/mdsn/examples/vue-starter/app/client.ts)

适合想让 Vue 接管组件树和视觉系统，但继续使用 MDSN 协议和运行时的场景。

## React 示例

参考：[examples/react-starter/app/client.tsx](/Users/hencoo/projects/mdsn/examples/react-starter/app/client.tsx)

适合想让 React 接管状态投影和交互组件，但继续复用 MDSN 运行时的场景。
