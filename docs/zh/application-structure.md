---
title: 应用结构
description: 使用 MDSN 构建真实应用或文档站时推荐的目录结构、路由模型与 action 组织方式。
---

# 应用结构

这页讲的是：如果你要用 MDSN 搭一个真实应用，代码、页面和交互最好怎么摆。

先说结论：MDSN 推荐把页面源、服务端逻辑和浏览器侧代码分开，但不要拆得太重。这样既容易理解，也不容易把一套很轻的应用做成两三套彼此分离的东西。
...
`index.mjs` 只是把这套应用托管起来，方便你在本地跑起来或部署到 Node 环境里。

## 先把职责分清

MDSN 里最容易写乱的地方，不是代码多少，而是职责混在一起。
...
- `createNodeHost()` 负责把这个应用挂到 Node 环境
...
- 用 `renderHtml` 生成全局外壳
- 或者在 Node 托管入口里用 `transformHtml` 做最终注入
