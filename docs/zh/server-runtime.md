...
- 共享的应用逻辑放在 `@mdanai/sdk/server`
- Node host 适配器放在 `@mdanai/sdk/server/node`
- Bun host 适配器放在 `@mdanai/sdk/server/bun`
...
- 通过 `@mdanai/sdk/server/node` 托管 Node `http`
- 通过 `@mdanai/sdk/server/bun` 托管 Bun
