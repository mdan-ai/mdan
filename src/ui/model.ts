...
export type { HeadlessSnapshot, MdanHeadlessUiHost };
...
export interface UiSnapshotView {
  status: HeadlessSnapshot["status"];
  error?: string;
  route?: string;
  markdown: string;
  blocks: UiBlockView[];
...
export async function submitUiOperation(
  host: Pick<MdanHeadlessUiHost, "submit" | "visit">,
  ...
  await dispatchOperation(host, operation.source, payload);
}
